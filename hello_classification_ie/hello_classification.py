...
def main():
    ...
    print("Total time :          %.3f ms, %.2f %%" % ((t8-t0)*1000,((t8-t0)/total_time *100)))
    print("inital time :         %.3f ms, %.2f %%" % ((t1-t0)*1000,((t1-t0)/total_time *100)))
    print("read model time :     %.3f ms, %.2f %%" % ((t2-t1)*1000,((t2-t1)/total_time *100)))
    print("configure i/o time :  %.3f ms, %.2f %%" % ((t3-t2)*1000,((t3-t2)/total_time *100)))    
    print("loading model time :  %.3f ms, %.2f %%" % ((t4-t3)*1000,((t4-t3)/total_time *100)))
    print("infer request time :  %.3f ms, %.2f %%" % ((t5-t4)*1000,((t5-t4)/total_time *100)))
    print("prepare time :        %.3f ms, %.2f %%" % ((t6-t5)*1000,((t6-t5)/total_time *100)))
    print("inference time :      %.3f ms, %.2f %%" % ((t7-t6)*1000,((t7-t6)/total_time *100)))
    print("process output time : %.3f ms, %.2f %%" % ((t8-t7)*1000,((t8-t7)/total_time *100)))
